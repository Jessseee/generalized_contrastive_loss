...
def get_city_idx(root_dir, city, name):
    ...
    for i in range(len(db_data_pp)):
        ...
        filename = os.path.join("train_val", city, name, "images", f"{key_pp}.jpg")
        assert os.path.exists(os.path.join(root_dir, filename)), f"{filename} not found"
    ...
    idx_file = os.path.join(root_dir, "train_val", city, f"{name}.json")
    with open(idx_file, "w") as f:
        json.dump(idx, f)
    print(f"{idx_file} created succesfully")
...
def create_idx_msls(root_dir):
    ...
    for f in sorted(os.listdir(data_path)):
        ...
        if os.path.isdir(city_path) and os.path.exists(os.path.join(city_path, "query/postprocessed.csv")):
            print(f"Creating json index for {f}")
...
def create_idx_scene(root_dir, scene, name):
    ...
    split_file = os.path.join(scene_path, f"{name.capitalize()}Split.txt")
    ...
    idx_file = os.path.join(root_dir, scene, f"{name}.json")
    with open(idx_file, "w") as frame:
        json.dump(idx, frame)
    print(f"{idx_file} created succesfully")
